...
Following the PC-side fit in `magnet_field_model/calibration/`:
...
- $\mathbf v = \mathbf s_i - \mathbf t$ — sensor position relative to the knob origin, global frame
- $\mathbf d = R^T\mathbf v - \mathbf m_j$ — magnet-to-sensor vector, knob frame
- $\mathbf b = R^T \mathbf B_g$ — the predicted field, knob frame
- $s$ — the magnet's strength

$$\frac{\partial \mathbf B_g}{\partial \mathbf m_j} = -M R
\qquad
\frac{\partial \mathbf B_g}{\partial \boldsymbol\varepsilon} = M R [\mathbf d]_\times - R[\mathbf b]_\times
\qquad
\frac{\partial \mathbf B_g}{\partial s} = \frac{\mathbf B_g}{s}$$

where $\boldsymbol\varepsilon$ is a **left** perturbation of the magnet's
rotation, $R_{mag} \to \exp([\boldsymbol\varepsilon]_\times)R_{mag}$ — the same
convention `Math.md` Identity 2 uses for the pose rotation, so tilt and pose
step the same way.

Derivation sketch: start from
$\mathbf B_g = R\,R_{mag}\,B\!\left(R_{mag}^T(R^T\mathbf v - \mathbf m_j)\right)$
and differentiate with respect to $\mathbf m_j$ and to $\boldsymbol\varepsilon$.
...
Note the tilt expression's second term. $R_{mag}$ cancels out of
$\mathbf b = R_{mag}^T R^T \mathbf B_g$ rotated back, which is why the formula
...
\qquad
\frac{\partial \hat{\mathbf B}_i}{\partial g_k} = G_k \mathbf B_{g,i}$$

where $G_k$ is the $k$-th traceless gain basis matrix. These belong to a
...
$\hat{\mathbf B} = G\mathbf B_g + \text{offset}$, so every derivative acting
through the field carries a factor of $G$:

$$\frac{\partial (G\mathbf B_g)}{\partial(\text{magnet param})} = G\,\frac{\partial \mathbf B_g}{\partial(\text{magnet param})}$$
...
for the pose Jacobian, where $[\mathbf B_g]_\times$ must be built from the
...
$$\frac{\partial \mathbf B}{\partial(\text{multiplier})} = \frac{\partial \mathbf B}{\partial s}\cdot s_{nominal}$$
...
the chart cannot reach since $n_z > 0$ for all finite $(u,v)$. The coordinates
...
$$\frac{\partial \boldsymbol\varepsilon}{\partial(u,v)} = [\hat n_{knob}]_\times\, R_{nominal}\, \frac{\partial \hat n_{local}}{\partial (u,v)}$$
...
`firmware/test/test_jacobian.cpp`. Two tiers are needed, and the second is the
