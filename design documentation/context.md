...
* **Overclocking:** Not currently configured — `platformio.ini` runs the RP2040 at its stock 133 MHz. PlatformIO can raise `board_build.f_cpu` to safely overclock it, which would cut the CPU-bound solver's execution time roughly proportionally with zero code changes; this is an available lever, not something the current build does.
