...
def format_cpp(geometry: BundleGeometry) -> str:
    ...
    Emits an `inline fittedCalibration()` function shaped like
    ...
    `inline` is load-bearing, not decoration: this snippet is meant to be
    pasted, and there is no way to guarantee it lands in exactly one .cpp
    file rather than a header that several translation units include. A
    non-inline free function with the same name and body compiled into more
    than one .o is a multiple-definition link error - `inline` is the
    standard C++ answer (one definition permitted per translation unit, the
    linker collapses them), and it is cheap here since the whole thing runs
    once, at startup.
    ...
    lines = [
        ...
        "// firmware/src/Config.cpp. inline because this may get pasted into a",
        "// header instead - see format_cpp()'s docstring.",
        "inline CalibrationParams fittedCalibration() {",
