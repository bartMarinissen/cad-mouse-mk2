...
Vec2 __not_in_flash_func(BicubicField::evaluate)(float r, float z, Mat2 &jacobian) const noexcept {
    ...
    // a_i(t) = 0.5 * sum_j BasisMatrix(i,j) * t^j (t^0=1). static constexpr:
    // constant-initialized into .rodata with no runtime init code, rather
    // than dynamically constructed on first call (TODO/eigen-to-bla-migration.md)
