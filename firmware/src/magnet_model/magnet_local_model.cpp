...
Vec3 __not_in_flash_func(dipole_field)(const Vec3& m, const Vec3& r, Mat3& J) {
    ...
    // field being curl-free away from its source. The formula itself:
    //
    //  J_ab = k[ 3 rho^-5  (m_a r_b + r_a m_b + (m·r) d_ab) -   15 (m·r) rho^-7 r_a r_b ]
    ...
    //      = 5 a3 (m.r) rho^-2
    //
    // This build sets -ffinite-math-only project-wide (platformio.ini), which
    // licenses folds like x*0.0f -> 0.0f -- the same mechanism that let
    // virtual_sensor.cpp's skew-matrix Jacobian and solve_pose.cpp's H=J^T*J
    // drop their hand-unrolled forms (TODO/Performance.md). Measured here too:
    // the full outer-product form below reaches 65 soft-float calls against
    // the hand-rolled 6-of-9-then-mirror form's 67 (isolated ARM disassembly,
    // real project flags) -- not just free, slightly cheaper, most likely
    // because the hand-rolled form's explicit "only compute six" bookkeeping
    // costs a little arithmetic the compiler's own CSE across the full
    // symmetric matrix already avoids for free.
