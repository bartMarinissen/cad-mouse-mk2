...
**Everything through the fourth pass describes the pre-cross-magnet call
graph.** `origin/experimental`'s forward-model refactor (every sensor sees
every magnet, not just its paired one) changed what `VirtualSensor::evaluate`
actually computes and added a new hand-unrolled Jacobian (`dipole_field`)
that didn't exist when the fourth pass ran. The fifth pass, near the bottom,
re-derives that pass's conclusions against the new call graph rather than
assuming they still hold, and checks the one genuinely new candidate.
...
## Fifth pass: cross-magnet refactor — redoing the checks after a significant math change

`origin/experimental` did a "full refactor of the forward-model call chain"
(every sensor now sees all three magnets by superposition: its own via the
existing bicubic near field, the other two as point dipoles) entirely
against Eigen, independently of everything above — it branched before the
BLA migration and never merged it. Re-derived onto BLA and re-verified onto
this document's own conclusions, rather than assuming they still held:
`virtual_sensor.cpp`'s skew-matrix Jacobian assembly now runs on a 3-way sum
instead of a single magnet's contribution, feeding two more calls
(`near_approx_world` + two `far_approx_world`s) into the same function ahead
of the skew assembly — exactly the kind of surrounding-code change that
could plausibly move an inlining/register-pressure boundary (see this
document's own note, fourth pass, about a leftover out-of-line
`BLA::operator*` call being attributed to "cost budget already spent
inlining other things in that function"). And the refactor added one
genuinely new piece of hand-unrolling — `dipole_field`'s Jacobian — that
never existed when the fourth pass's "remove all hand-unrolling" decision
was made, so it had never been checked at all.

**Method, per the user's explicit instruction this time: no re-exploring
flag-scoping mechanisms.** That question is closed (fourth pass): project-wide
`-O3 -ffinite-math-only`, real command-line flags, is the only mechanism
that reaches parity. Just compile the hand-rolled and clean forms with the
real project's exact flags (`pio run -v`), isolate via a temporary in-place
edit + rebuild (same method as the fourth pass), disassemble
(`arm-none-eabi-objdump -d -C`), count `bl __aeabi_f*`/`bl sqrtf` calls, then
revert. All four checks below used the identical real build (same
`platformio.ini`, no scratch flag variants).

**(a) `dipole_field`'s Jacobian — the one candidate with no prior
precedent.** Hand-rolled: 6-of-9-entries-computed-then-3-mirrored, using
factored constants (`a3`, `a15`) to avoid recomputing shared terms. Clean
form: the unreduced `a3*(outer(m,r) + outer(r,m)^T + (m·r)*I) - a15*outer(r,r)`
from the function's own header docstring, using a new `outer()` helper
(`math3D.h`, alongside `dot()`/`cwise_product()`/`cwise_sqrt()` in the same
BLA-gap-filler group).

| form | soft-float calls |
|---|---:|
| hand-rolled (6-of-9 + mirror) | 67 |
| clean (unreduced outer-product) | **65** |

**Not just free — slightly cheaper.** The most likely explanation: the
hand-rolled form's explicit "only compute six, mirror three" bookkeeping
costs a little arithmetic of its own, which the compiler's own CSE across
the full symmetric matrix already avoids without being told to. Kept the
clean form; real flash dropped 16 B project-wide as a result (not a
measurement artifact — the whole-project build is 16 B smaller with this
one change, consistent with 2 fewer calls).

**(b) `virtual_sensor.cpp`'s skew-matrix assembly.** Same code shape as the
fourth pass's already-proven-parity form (`M * skew_matrix(v) -
skew_matrix(B)`), now fed the 3-way-summed `J_displacement_world`/`B_world`
with two more function calls ahead of it in the same function body — the
scenario flagged above as worth re-checking rather than assuming.

| form | soft-float calls (whole `VirtualSensor::evaluate`) |
|---|---:|
| hand-rolled skew loop | 60 |
| clean (`skew_matrix()` general form) | 60 |

**Exact parity, unchanged.** The register-pressure/inlining-budget worry
did not materialize here — worth having checked rather than assumed, but
the fourth pass's conclusion holds under the new call shape.

**(c) `BicubicField.cpp`'s Catmull-Rom basis weights.** Math body identical
to the fourth pass's measurement (still the same 16-line hand-expanded
scalar polynomial on `origin/experimental`, since that branch's refactor
only collapsed the *out-param signature* — three separate `Vec2&`s to one
`Mat2&` — never touched the arithmetic). Re-measured under the new
signature rather than trusting the old 344-vs-320 numbers unchecked, since
the out-param write now goes through `.Column()`/`RefMatrix` assignment
instead of a bare reference write:

| form | soft-float calls |
|---|---:|
| hand-expanded scalar | 328 |
| clean (`Mat4`/`Vec4` matrix form) | 352 |

**~7.3% more (24 calls)** — consistent with the fourth pass's ~7.5% (24
calls on a smaller 320/344 base), not a new regression introduced by the
signature change. Kept per the standing "remove all hand-unrolling"
decision, same as before.

**(d) `solve_pose.cpp`'s `H = JᵀJ`.** Structurally unaffected by the
cross-magnet refactor — `jacobian` is still 9x6, nothing about the
summation moved into this file. Re-checked anyway rather than trusting
"should be fine" reasoning, since the instruction was to redo the checks:

| form | soft-float calls (whole `solve_knob_pose`) |
|---|---:|
| hand-rolled 21-dot-product lower-triangle | 504 |
| clean (`~jacobian * jacobian`) | 504 |

**Exact parity, unchanged**, as expected — the fastest of the four to
confirm.

**Net result: all four candidates land on the clean `BLA` form.** Three of
four are free-or-better; the fourth (BicubicField) carries the same real,
honestly-documented ~7.3-7.5% cost it always has, kept for the same reason.

**Full verification** (all after this pass's changes, on the real branch):
`pio test -e native_test` (host-native, all 8 `test_jacobian.cpp` cases
including the new `test_dipole_field_jacobian`/
`test_dipole_matches_the_magnet_the_table_models`/
`test_cross_magnet_terms_are_actually_present`) passing; a synthetic
ground-truth solver-convergence check against the new 3-magnet
`ForwardModel` (hot-start-sized perturbation, ~0.8mm/~3° — recovered to
|t_err| ≈ 3.3×10⁻⁵ mm, rotation error ≈ 0.028°, matching this document's
prior-pass precision); `pio run -e seeed_xiao_rp2040` and `-e
seeed_xiao_rp2040_unity` both build clean; `pio test -e
seeed_xiao_rp2040_test --without-uploading --without-testing` builds clean
(no hardware attached in this sandbox — see "Open next steps").
...
- **Cross-magnet refactor's real cost, on-device.** The fifth pass measured
  soft-float call counts, not wall-clock — same limitation as everywhere
  else in this document without hardware. `TODO/resolved/cross-magnet-interference.md`
  already flags this: the refactor is not yet verified on real hardware,
